...
async function createSend(config) {
    ...
    return async (operation) => {
        ...
        await doc_1.default.validate(data, Context.context);
        ...
        await Promise.all(flattened.map(async (doc) => {
            ...
            return sendFn(Object.assign({}, operation, { data: doc, key: doc["@id"] }));
        }));
    };
}
...
async function createReceive(config) {
    ...
    const _receive = async (operation) => {
        const { data } = operation;
        const expanded = (await doc_1.default.expand(data, Context.context))[0];
